...
EXPORT_SYMBOL(hw1_block_number);
int hw1_index = 0;
...
EXPORT_SYMBOL(hw1_file_system_name);
...
blk_qc_t submit_bio(int rw, struct bio *bio)
{
	...
	if (bio_has_data(bio)) {
		...
		if (rw & WRITE) {
			...
			if((unsigned long long) bio->bi_iter.bi_sector != NULL){
				hw1_block_number[hw1_index] = (unsigned long long) bio->bi_iter.bi_sector;
				do_gettimeofday(&mytime);
				hw1_time[hw1_index] = (unsigned long long)(mytime.tv_sec) * 1000000 + (unsigned long long)(mytime.tv_usec);
				if(bio->bi_bdev != NULL && bio->bi_bdev->bd_super != NULL && bio->bi_bdev->bd_super->s_type != NULL && bio->bi_bdev->bd_super->s_type->name != NULL)
					hw1_file_system_name[hw1_index] = bio->bi_bdev->bd_super->s_type->name;
